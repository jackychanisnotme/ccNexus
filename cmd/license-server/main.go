...
<body>
  ...
  <script>
    ...
    async function remoteRevealSecret(index,endpointName,credentialId,field){const device=window.deviceRows[index];try{if(!window.isSecureContext||!crypto.subtle){throw new Error('查看明文需要 HTTPS 安全后台；当前 HTTP 页面只允许远程维护，不展示密钥明文。')}const keyPair=await createRevealKeyPair();const command=await api('/api/admin/devices/'+encodeURIComponent(device.deviceId)+'/remote/secrets/reveal',{method:'POST',body:JSON.stringify({endpointName,credentialId,field,adminPublicKey:keyPair.publicKey})});message.textContent='已下发一次性查看命令，等待客户端回传';const done=await pollRemoteCommand(device.deviceId,command.id,keyPair);await loadRemoteDetail(index);if(done&&done.value){alert('一次性明文（请立即使用，勿转存）：\n'+done.value)}}catch(err){setError(err)}}
    async function pollRemoteCommand(deviceId,commandId,keyPair){const deadline=Date.now()+65000;while(Date.now()<deadline){await sleep(1500);let command;try{command=await api('/api/admin/devices/'+encodeURIComponent(deviceId)+'/remote/commands/'+commandId)}catch(err){if(String(err.message||err).includes('expired'))throw new Error('远程命令结果已过期');throw err}if(command.status==='queued'){message.textContent='远程命令等待客户端拉取';continue}if(command.status==='failed')throw new Error(command.error||'远程命令执行失败');if(command.status==='applied'){message.textContent='远程命令已应用';const reveal=command.resultJson&&command.resultJson.secretReveal;if(reveal&&keyPair){return await decryptRevealResult(keyPair,reveal)}return command}}throw new Error('客户端未在 65 秒内回传结果，可能离线或版本不支持')}
