...
<body>
  ...
  <script>
    ...
    async function loadRemoteDetail(index){const box=document.getElementById('remote-detail-'+index);const device=window.deviceRows[index];if(!box||!device)return;box.innerHTML='<div class="detail-label">远程端点维护</div><div class="muted">加载中</div>';try{const data=await api('/api/admin/devices/'+encodeURIComponent(device.deviceId)+'/remote');window.remoteDetailData=window.remoteDetailData||{};window.remoteDetailData[index]=data;box.innerHTML=renderRemoteDetail(index,device,data)}catch(err){box.innerHTML='<div class="detail-label">远程端点维护</div><div class="status-disabled">'+esc(err.message||err)+'</div>'}}
    function renderRemoteDetail(index,device,data){const state=(data&&data.state)||{};if(!state.supported)return '<div class="detail-label">远程端点维护</div><div class="muted">该客户端版本暂不支持远程维护，不影响正常授权和本地使用。</div>';const snap=state.snapshot||{};const endpoints=snap.endpoints||[];const pools=snap.tokenPools||[];const remoteStatus='<div class="detail-label">远程端点维护</div><div class="muted">状态：'+(state.enabled?'已启用':'已关闭')+' · 版本：'+esc(state.clientVersion||'-')+' · 心跳：'+dt(state.lastHeartbeatAt)+'</div>'+(can('devices:remote:write')?'<div class="actions"><button class="small-btn" onclick="remoteCreateEndpoint('+index+')">新增端点</button></div>':'');const endpointRows=endpoints.length?endpoints.map((ep,epIndex)=>'<tr><td>'+esc(ep.name)+'</td><td>'+esc(ep.enabled?'启用':'停用')+'</td><td class="mono">'+esc(ep.apiUrl||'-')+'</td><td class="mono">'+esc(ep.apiKeyMasked||'-')+'</td><td>'+esc(ep.authMode||'-')+'</td><td>'+esc((ep.maxConcurrentRequests||0)>0?ep.maxConcurrentRequests:'不限')+'</td><td>'+remoteStats(ep.stats)+'</td><td><div class="actions">'+(can('devices:remote:write')?'<button class="secondary small-btn" '+(epIndex===0?'disabled ':'')+'onclick="remoteMoveEndpoint('+index+','+epIndex+',-1)">上移</button><button class="secondary small-btn" '+(epIndex===endpoints.length-1?'disabled ':'')+'onclick="remoteMoveEndpoint('+index+','+epIndex+',1)">下移</button><button class="secondary small-btn" onclick="remoteUpdateEndpoint('+index+',&quot;'+escAttr(ep.name)+'&quot;,&quot;apiUrl&quot;)">改URL</button><button class="secondary small-btn" onclick="remoteUpdateEndpoint('+index+',&quot;'+escAttr(ep.name)+'&quot;,&quot;apiKey&quot;)">改Key</button><button class="secondary small-btn" onclick="remoteUpdateEndpoint('+index+',&quot;'+escAttr(ep.name)+'&quot;,&quot;maxConcurrentRequests&quot;)">改并发</button><button class="secondary small-btn" onclick="remoteToggleEndpoint('+index+',&quot;'+escAttr(ep.name)+'&quot;,'+(!ep.enabled)+')">'+(ep.enabled?'停用':'启用')+'</button><button class="danger small-btn" onclick="remoteDeleteEndpoint('+index+',&quot;'+escAttr(ep.name)+'&quot;)">删除</button>':'')+(can('devices:remote:secrets')?'<button class="secondary small-btn" onclick="remoteRevealSecret('+index+',&quot;'+escAttr(ep.name)+'&quot;,0,&quot;apiKey&quot;)">查看Key</button>':'')+'</div></td></tr>').join(''):'<tr><td colspan="8" class="empty">暂无端点快照</td></tr>';const poolRows=pools.length?pools.map(pool=>'<tr><td>'+esc(pool.endpointName)+'</td><td colspan="6">'+renderRemoteCredentials(index,pool)+'</td></tr>').join(''):'<tr><td colspan="7" class="empty">暂无 Token Pool 快照</td></tr>';return remoteStatus+'<table><thead><tr><th>端点</th><th>状态</th><th>Base URL</th><th>API Key</th><th>模式</th><th>并发</th><th>用量</th><th>操作</th></tr></thead><tbody>'+endpointRows+'</tbody></table><div class="detail-label" style="margin-top:12px">Codex Token Pool</div><table><thead><tr><th>端点</th><th colspan="6">账号/额度/维护</th></tr></thead><tbody>'+poolRows+'</tbody></table>'}
