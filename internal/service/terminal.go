...
// RepairCodexSessionVisibility repairs local Codex sidebar visibility metadata.
func (t *TerminalService) RepairCodexSessionVisibility(requestJSON string) string {
	var req session.CodexVisibilityRepairRequest
	if requestJSON != "" {
	...
	summary, err := session.RepairCodexSessionVisibility(req)
	if err != nil {
		result := map[string]interface{}{
			"success": false,
			"error":   err.Error(),
