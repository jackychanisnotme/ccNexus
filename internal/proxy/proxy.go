...
type Proxy struct {
	...
	runtime                     atomic.Pointer[proxyRuntimeSnapshot]
	...
	credentialRefreshCalls      map[int64]*credentialRefreshCall
...
func New(cfg *config.Config, statsStorage StatsStorage, sqliteStorage *storage.SQLiteStorage, deviceID string) *Proxy {
	...
	p := &Proxy{
		...
		credentialRefreshCalls:  make(map[int64]*credentialRefreshCall),
...
func (p *Proxy) StartWithMux(customMux *http.ServeMux) error {
	p.listenerMu.Lock()
	if p.server != nil && !p.serverStopped {
		p.listenerMu.Unlock()
		return fmt.Errorf("proxy server already started")
	}

	handler := p.ensureHTTPHandlerLocked(customMux)
	...
	p.listener = ln
	p.listenerAddr = addr
	p.serverStopCh = make(chan struct{})
	p.serverFatalCh = make(chan error, 1)
	p.serverStopped = false
	stopCh := p.serverStopCh
...
func (p *Proxy) ensureHTTPHandlerLocked(customMux *http.ServeMux) http.Handler {
	...
	p.httpHandler = p.trackInboundConnections(mux)
	return p.httpHandler
}

func (p *Proxy) serveListener(ln net.Listener, addr string) {
	go func() {
		...
		p.listenerMu.Lock()
		isCurrent := p.listener == ln && !p.serverStopped
		fatalCh := p.serverFatalCh
		p.listenerMu.Unlock()
...
// RebindListener replaces the active listener with one built from the current
// listen configuration. If the new bind fails, it restores the previous listener.
func (p *Proxy) RebindListener() error {
	if p == nil {
		return fmt.Errorf("proxy is nil")
	}

	var listenerToServe net.Listener
	...
	oldAddr := p.listenerAddr
	newAddr := p.config.GetListenAddr()
	if oldAddr == newAddr {
		p.listenerMu.Unlock()
		return nil
	}
	...
	if err != nil {
		restoreListener, restoreErr := net.Listen("tcp", oldAddr)
		if restoreErr == nil {
			p.listener = restoreListener
			p.listenerAddr = oldAddr
			p.server.Addr = oldAddr
			listenerToServe = restoreListener
			listenerAddr = oldAddr
		}
		p.listenerMu.Unlock()

		if listenerToServe != nil {
			p.serveListener(listenerToServe, listenerAddr)
		}
		if restoreErr != nil {
			return fmt.Errorf("failed to bind listener on %s: %w; failed to restore listener on %s: %v", newAddr, err, oldAddr, restoreErr)
		}
		return fmt.Errorf("failed to bind listener on %s: %w", newAddr, err)
	}

	p.listener = newListener
	p.listenerAddr = newAddr
	p.server.Addr = newAddr
	listenerToServe = newListener
	listenerAddr = newAddr
	p.listenerMu.Unlock()
...
func (p *Proxy) getEnabledEndpoints() []config.Endpoint {
	return enabledEndpoints(p.runtimeSnapshot().endpoints)
}

func enabledEndpoints(allEndpoints []config.Endpoint) []config.Endpoint {
...
func (p *Proxy) getCurrentEndpointLocked() config.Endpoint {
	...
	endpoints := enabledEndpoints(runtimeSnapshot.endpoints)
	if len(endpoints) == 0 {
...
func (p *Proxy) handleProxy(w http.ResponseWriter, r *http.Request) {
	...
	// 尝试解析客户端指定的端点
	specifiedEndpoint, requestedModelSuffix, resolveErr := runtimeSnapshot.resolver.ResolveEndpoint(r, bodyBytes)
	if resolveErr != nil {
		// 端点指定错误，返回错误响应
	...
	for retry := 0; retry < maxRetries; retry++ {
		...
		if config.IsTokenPoolAuthMode(authMode) {
			...
			if credential == nil {
				...
				p.recordEndpointErrorForClient(endpoint.Name, "no_usable_token", obs.ClientIP)
				p.markRequestInactive(endpoint.Name)
				if endpointAttempts >= endpointFastFailoverAttempts || p.isEndpointInActiveCooldown(endpoint.Name) {
			...
			if selectedCredential != nil {
				...
				credentialAccessToken = selectedCredential.AccessToken
			}
			if apiKey == "" {
				logRequestAttemptWarn(obs, endpoint.Name, attemptNumber, 0, "no_usable_token", "No usable token in token pool")
				p.recordEndpointErrorForClient(endpoint.Name, "no_usable_token", obs.ClientIP)
			...
			p.recordEndpointErrorForClient(endpoint.Name, "empty_api_key", obs.ClientIP)
			p.markRequestInactive(endpoint.Name)
			if endpointAttempts >= endpointFastFailoverAttempts || p.isEndpointInActiveCooldown(endpoint.Name) {
		...
		if err != nil {
			...
			p.recordEndpointErrorForClient(endpoint.Name, "prepare_transformer_failed", obs.ClientIP)
			p.markRequestInactive(endpoint.Name)
			if endpointAttempts >= endpointFastFailoverAttempts || p.isEndpointInActiveCooldown(endpoint.Name) {
		...
		if err != nil {
			...
			p.recordEndpointErrorForClient(endpoint.Name, "transform_request_failed", obs.ClientIP)
			p.markRequestInactive(endpoint.Name)
			if endpointAttempts >= endpointFastFailoverAttempts || p.isEndpointInActiveCooldown(endpoint.Name) {
		...
		if err != nil {
			...
			p.recordEndpointErrorForClient(endpoint.Name, "build_request_failed", obs.ClientIP)
			p.markRequestInactive(endpoint.Name)
			if endpointAttempts >= endpointFastFailoverAttempts || p.isEndpointInActiveCooldown(endpoint.Name) {
		...
		proxyURL := resolveProxyURLForRequest(runtimeSnapshot.config, proxyReq.URL, endpoint)
		...
		if useHTTPUpstream && err == nil {
			resp, err = sendRequestWithResponseHeaderTimeout(ctx, proxyReq, endpoint, p.httpClient, runtimeSnapshot.config, responseHeaderTimeout, !upstreamStreaming)
		}
		if err != nil {
			...
			p.markCredentialFailureIfCurrent(credentialID, credentialAccessToken, 0, err.Error())
		...
		if resp.StatusCode == http.StatusOK && !streamReq.Stream && shouldAggregateStreamingAsNonStreaming(endpoint, transformerName) {
			...
			if err == nil {
				...
				p.markCredentialSuccessIfCurrent(credentialID, credentialAccessToken)
			...
			p.markCredentialFailureIfCurrent(credentialID, credentialAccessToken, 0, err.Error())
			p.recordCredentialUsage(credentialID, endpoint.Name, 0, 1, 0, 0)
			p.recordEndpointErrorForClient(endpoint.Name, "aggregate_streaming_failed", obs.ClientIP)
			p.markRequestInactive(endpoint.Name)
			if endpointAttempts >= endpointFastFailoverAttempts || p.isEndpointInActiveCooldown(endpoint.Name) {
				...
				if errors.As(streamResult.Err, &webSocketUpstreamErr) {
					if webSocketUpstreamErr.StatusCode == http.StatusTooManyRequests {
						...
						if retryAt, limited := storage.CodexRateLimitRetryAt(data, now); limited {
							...
							p.markCredentialFailureIfCurrent(credentialID, credentialAccessToken, webSocketUpstreamErr.StatusCode, webSocketUpstreamErr.Error())
						}
						p.recordCredentialUsage(credentialID, endpoint.Name, 0, 1, 0, 0)
						p.markRequestInactive(endpoint.Name)
		...
		if resp.StatusCode == http.StatusOK {
			...
			p.markCredentialFailureIfCurrent(credentialID, credentialAccessToken, 0, err.Error())
			p.recordCredentialUsage(credentialID, endpoint.Name, 0, 1, 0, 0)
			p.recordEndpointErrorForClient(endpoint.Name, "non_stream_response_failed", obs.ClientIP)
			p.markRequestInactive(endpoint.Name)
			if endpointAttempts >= endpointFastFailoverAttempts || p.isEndpointInActiveCooldown(endpoint.Name) {
		...
		if shouldRetry(resp.StatusCode) {
			...
			if shouldTreatAPIKeyEndpointAuthFailure(authMode, resp.StatusCode, errMsg) {
				p.recordEndpointErrorForClient(endpoint.Name, retryReasonEndpointAuthFailed, obs.ClientIP, resp.StatusCode)
				if !useSpecificEndpoint && requestPlan.Len() > 1 {
			...
			p.markCredentialFailureIfCurrent(credentialID, credentialAccessToken, resp.StatusCode, errMsg)
			p.recordCredentialUsage(credentialID, endpoint.Name, 0, 1, 0, 0)
			p.recordEndpointErrorForClient(endpoint.Name, retryReason, obs.ClientIP, resp.StatusCode)
			p.markRequestInactive(endpoint.Name)
		...
		if (resp.StatusCode == http.StatusUnauthorized || resp.StatusCode == http.StatusForbidden) && credentialID > 0 {
			...
			if skipCredentialPenalty {
				...
				if selectedCredential != nil &&
					...
					if errors.Is(refreshErr, errCodexRefreshTokenReused) {
						p.markCredentialFailureIfCurrent(credentialID, credentialAccessToken, http.StatusUnauthorized, refreshErr.Error())
				...
				p.markCredentialFailureIfCurrent(credentialID, credentialAccessToken, resp.StatusCode, respLogMsg)
				p.recordCredentialUsage(credentialID, endpoint.Name, 0, 1, 0, 0)
				p.recordEndpointErrorForClient(endpoint.Name, "credential_auth_failed", obs.ClientIP, resp.StatusCode)
				p.markRequestInactive(endpoint.Name)
				endpointAttempts = 0
		...
		if resp.StatusCode != http.StatusOK {
			...
			if skipCredentialPenalty {
				p.markCredentialFailureIfCurrent(credentialID, credentialAccessToken, 0, respLogMsg)
				p.recordCredentialUsage(credentialID, endpoint.Name, 0, 1, 0, 0)
			} else {
				p.markCredentialFailureIfCurrent(credentialID, credentialAccessToken, resp.StatusCode, respLogMsg)
				p.recordCredentialUsage(credentialID, endpoint.Name, 0, 1, 0, 0)
			...
			if !skipCredentialPenalty && !endpointAuthFailureHandled {
				p.recordEndpointErrorForClient(endpoint.Name, "non_retryable_status", obs.ClientIP, resp.StatusCode)
			}
...
func (p *Proxy) markCredentialSuccessIfCurrent(credentialID int64, expectedAccessToken string) {
	if credentialID <= 0 || p.storage == nil {
		return
	}
	updated, err := p.storage.MarkCredentialSuccessIfCurrent(credentialID, expectedAccessToken, time.Now().UTC())
	if err != nil {
		logger.Warn("Failed to mark credential success (id=%d): %v", credentialID, err)
	} else if !updated {
		logger.Debug("Ignored stale credential success (id=%d)", credentialID)
	}
}
...
func (p *Proxy) markCredentialFailureIfCurrent(credentialID int64, expectedAccessToken string, statusCode int, errMsg string) {
	if credentialID <= 0 || p.storage == nil {
		return
	}
	updated, err := p.storage.MarkCredentialFailureIfCurrent(credentialID, expectedAccessToken, statusCode, errMsg, time.Now().UTC())
	if err != nil {
		logger.Warn("Failed to mark credential failure (id=%d): %v", credentialID, err)
	} else if !updated {
		logger.Debug("Ignored stale credential failure (id=%d)", credentialID)
	}
}
...
func (p *Proxy) recordSemanticEmptyResponseFailure(endpointName string, credentialID int64, clientIP string, err *semanticEmptyResponseError, recordEndpointFailure bool) {
	if err == nil {
